...
##### 内联函数、构造函数、静态成员函数可以吗？都不行

+ inline 是编译时展开，必须有实体

+ constructor 函数如果是 virtual 的，调用时也需要根据 vtable 寻找，但是 constructor 执行前没有初始化 vptr，显然就不行。而且， 在构造函数中国调用虚函数，实际上执行的是父类的对应函数，因为自己这个类还没有构造好，多态是被 disable 的。

+ static 属于 class 自己的，必须有实体；
