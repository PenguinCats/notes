...
## Linux 网络子系统中 网卡 DMA 的一些细节

> Linux网络子系统中DMA机制的实现(超详细!) - 玩转Linux内核的文章 - 知乎
> https://zhuanlan.zhihu.com/p/464204319

### 1. DMA机制在TCP/IP协议模型中的位置

主存的内核空间中为接收和发送数据分别建立了两个环形缓冲区（Ring Buffer）。分别叫接受环形缓冲区（Receive Ring Buffer）和发送环形缓冲区（Send Ring Buffer），通常也叫DMA环形缓冲区。DMA机制位于TCP/IP协议模型中的位置数据链路层。

网卡通过DMA方式将数据发送到**Receive Ring Buffer**,然后**Receive Ring Buffer**把数据包传给IP协议所在的网络层，然后再由路由机制传给TCP协议所在的传输层，最终传给用户进程所在的应用层。

![preview](https://pic1.zhimg.com/v2-4e0028587b003c51575197cac534ed8c_r.jpg)
...
![preview](https://pic1.zhimg.com/v2-cfa75207821406005b0990cfd2e4f724_r.jpg)
...
> - 4.poll 函数对数据进行检查，例如将几个 sk_buff 合并，因为可能同一个数据包被分散放在多个 sk_buff 中。
> - 5.poll 函数将 sk_buff 交付上层网络栈处理。
> - poll 函数清理 sk_buff，**清理 Ring Buffer 上的 Descriptor 将其指向新分配的 sk_buff**  并将状态设置为 ready。

> dma基础_一文读懂dma的方方面面 - Haonan的文章 - 知乎 https://zhuanlan.zhihu.com/p/413978652
> 
> 顺便一提之前学到的零拷贝技术，有被问道过，内存中用于 DMA 映射的空间一共就那么一点大，要怎么发出去呢？
> 
> 这里我猜测一下，不是一次性全部读到 DMA 缓冲区的？是读一部分发一部分？反正磁盘读来的数据也可能是先放在 DMA 缓冲区内的。这样的话可以整一点什么**流式 DMA**？？？
> 
> 这样带来的一个问题就是：这些缓冲区可能不连续啊！诶对，所以需要网卡支持 `scatter/gather DMA` 技术，可以从不连续的区域来读取。需要发送一个 `scatter/gather list` 来告诉 DMA 设备每块地址的起始地址和长度。
> 
> Scatter/gather is used to do DMA data transfers of data that is written to noncontiguous areas of memory.
> 
